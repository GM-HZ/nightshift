# NightShift V4.2 统一规格说明

**日期：** 2026-03-27

**状态：** 在 V4.1 基础上进一步收紧架构边界、状态模型与引擎契约后的统一规格版本。当前作为主设计目标，V1、V2、V3 保留为历史迭代版本。
...
- **Issue Registry**
  负责规范化 issue contract 以及 issue 级生命周期状态。

- **Run Orchestrator**
  负责队列选择、预算、重试、轮转和 run 级控制流。

- **Engine Adapter Layer**
  负责引擎能力声明、引擎调用以及结果归一化。

- **Validation Gate**
  负责独立验证与 acceptance / rejection 决策。

以下模块被明确放在稳定内核之外，保持为可替换边缘模块：
...
这样可以让执行内核保持小而稳定，同时允许产品表层更自由地演进。
...
V4.2 不再把所有状态混在一个扁平 `status` 字段里，而是明确拆分状态域。

### 13.1 IssueState

描述这个 issue 本身是否适合继续被调度或恢复。
...
每个 issue 记录都应分别保存这些状态域，而不是把所有语义压进一个过载的 `status` 字段。
...
6. 收集引擎输出。
7. 运行独立验证。
8. 基于执行与验证结果更新 `AttemptState`。
9. 按需更新 `IssueState`、`DeliveryState`、`blocker_type` 与 `progress_type`。
...
### 16.1 Engine Capabilities 契约

Harness 在调度前必须知道一个引擎“能做什么”。
...
能力声明应被视为 adapter 契约的一部分，而不是隐藏实现细节。

### 16.2 调度规则

Harness 应根据 engine capabilities 决定：

- 某个引擎是否适合当前 issue
- 是否可以进行 fallback
- 是否能启用 resume、structured output 等能力
...
交付层状态迁移应通过 `DeliveryState` 表达，而不是直接复用执行态字段。
...
MVP 仍然可以以 current-state 文件为中心，但设计上应为 append-only event capture 预留空间。
...
- 拆分后的 issue / attempt / delivery state model
- queue 与 run control
- Codex CLI adapter
...
相比 V4.1，V4.2 进一步收紧了三个核心架构接缝：

- issue / attempt / delivery 三层状态模型
- engine capability contract 与 normalized outcome 双层契约
- 更小、更稳定的内核模块边界
