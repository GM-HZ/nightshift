...
def test_evaluate_deliverability_accepts_done_and_accepted_issue(tmp_path: Path) -> None:
    ...
    result = evaluate_deliverability(
        make_contract("GH-7"),
        make_record("GH-7", worktree),
...
def test_evaluate_deliverability_rejects_non_accepted_issue(tmp_path: Path) -> None:
    ...
    result = evaluate_deliverability(make_contract("GH-7"), record, changed_paths=("README.md",))
...
def test_evaluate_deliverability_rejects_missing_worktree(tmp_path: Path) -> None:
    ...
    result = evaluate_deliverability(make_contract("GH-7"), make_record("GH-7", missing), changed_paths=("README.md",))

    assert result.allowed is False
    assert "worktree" in result.reason


def test_evaluate_deliverability_rejects_empty_changed_paths(tmp_path: Path) -> None:
    worktree = tmp_path / ".worktrees" / "issue-GH-7"
    worktree.mkdir(parents=True)

    result = evaluate_deliverability(make_contract("GH-7"), make_record("GH-7", worktree), changed_paths=())

    assert result.allowed is False
    assert "no staged changes" in result.reason


def test_evaluate_deliverability_rejects_paths_outside_allowed_paths(tmp_path: Path) -> None:
    worktree = tmp_path / ".worktrees" / "issue-GH-7"
    worktree.mkdir(parents=True)
    ...
    assert result.allowed is False
    assert "outside allowed_paths" in result.reason
