...
from nightshift.product.delivery.git_ops import git_add_all, git_commit, git_push, render_commit_message
...
class DeliveryService:
    def __init__(
        ...
        *,
        ...
        git_add: Callable[[str | Path], None] = git_add_all,
    ...
    def _deliver_one(self, issue_id: str) -> DeliveryResult:
        ...
        admission = evaluate_deliverability(contract, record)
        if not admission.allowed:
            raise SelectionError(admission.reason or f"issue {issue_id} is not deliverable")

        worktree_path = Path(record.worktree_path or "")
        try:
            self.git_add(worktree_path)
            ...
            pr_payload = render_pr_payload(
                ...
                issue_id=issue_id,
                source_issue_number=_parse_source_issue_number(issue_id),
...
def _parse_source_issue_number(issue_id: str) -> int | None:
    if "-" not in issue_id:
        return None
