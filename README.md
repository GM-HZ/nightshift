...
- Live operator surface: kernel-first plus GitHub issue ingestion bridge, queue admission, work-order materialization, batch execution via `run --issues` / `run --all`, unattended loop control via `run --all --daemon` / `stop`, and explicit delivery via `deliver --issues`
...
- unattended overnight control via `run --all --daemon` and `stop`
